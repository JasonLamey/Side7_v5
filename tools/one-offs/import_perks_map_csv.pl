...
sub import_csv_file
{
    ...
    if ( $fh->open("< $file") )
    {
        ...
        {
            say "\t>> Checking on perk >$row->{'Perk'}<";
            next if $row->{'Perk'} eq 'Perk'; # Skip any header line.

            my $perk = Side7::User::Perk->new( name => $row->{'Perk'} );
            my $loaded = $perk->load( speculative => 1 );

            if ( $loaded == 0 )
            {
                say "\tCould not load perk >$row->{'Perk'}< from the database.";
                next;
            }
            ...
            foreach my $role ( keys %roles )
            {
                ...
                {
                    say "\t\t>> Associating >$row->{'Perk'}< with >$role<" if $verbose;
                    my $role_perk_map = Side7::User::RolesPerksMap->new(
