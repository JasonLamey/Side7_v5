...
my $new_user_dir = $CONFIG->{'general'}->{'base_gallery_directory'} . $tier1 . '/' . $tier2 . '/' . $NEW_USER_ID;
...
is( -d $new_user_dir, 1, 'New User Directory >' . $new_user_dir . '< creation confirmed.' );
...
is( rmdir( $new_user_dir ), 1, 'Removed New User Directory >' . $new_user_dir . '<' )
    || diag( 'Received this error: ' . $! );
